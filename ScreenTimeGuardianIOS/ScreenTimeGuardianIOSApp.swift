...
struct SettingsView: View {
    ...
    #endif
    @State private var snapshotBeforeEdit: AppSettings? = nil

    private func takeSnapshot() {
        snapshotBeforeEdit = store.settings
    }

    private func restoreSnapshot() {
        guard let snapshot = snapshotBeforeEdit else { return }
        store.settings = snapshot
        store.saveSettings()
        snapshotBeforeEdit = nil
    }
    ...
    var body: some View {
        ...
        NavigationStack {
            Form {
                ...
                Section(localizedText("本周计划", "Weekly Plan", language: language)) {
                    HStack {
                        ...
                        // Hours stepper
                        Button {
                            let cur = store.settings.plannedDailyMinutes ?? defaultPlannedDailyMinutes
                            if cur / 60 > 0 { setPlannedDaily(hours: cur / 60 - 1, minutes: nil) }
                        } label: {
                            Image(systemName: "minus.circle.fill").font(.title3)
                                .foregroundStyle((store.settings.plannedDailyMinutes ?? defaultPlannedDailyMinutes) / 60 > 0 ? .blue : .gray.opacity(0.3))
                        }
                        .buttonStyle(.plain)
                        TextField("", value: plannedDailyHoursBinding, format: .number)
                            .keyboardType(.numberPad)
                            .multilineTextAlignment(.center)
                            .frame(maxWidth: 40)
                        Button {
                            let cur = store.settings.plannedDailyMinutes ?? defaultPlannedDailyMinutes
                            if cur / 60 < 24 { setPlannedDaily(hours: cur / 60 + 1, minutes: nil) }
                        } label: {
                            Image(systemName: "plus.circle.fill").font(.title3)
                                .foregroundStyle((store.settings.plannedDailyMinutes ?? defaultPlannedDailyMinutes) / 60 < 24 ? .blue : .gray.opacity(0.3))
                        }
                        .buttonStyle(.plain)
                        ...
                        Button {
                            let cur = store.settings.plannedDailyMinutes ?? defaultPlannedDailyMinutes
                            let rem = cur % 60
                            if rem > 0 { setPlannedDaily(hours: nil, minutes: rem - 1) }
                        } label: {
                            Image(systemName: "minus.circle.fill").font(.title3)
                                .foregroundStyle((store.settings.plannedDailyMinutes ?? defaultPlannedDailyMinutes) % 60 > 0 ? .blue : .gray.opacity(0.3))
                        }
                        .buttonStyle(.plain)
                        TextField("", value: plannedDailyMinuteRemainderBinding, format: .number)
                            .keyboardType(.numberPad)
                            .multilineTextAlignment(.center)
                            .frame(maxWidth: 40)
                        Button {
                            let cur = store.settings.plannedDailyMinutes ?? defaultPlannedDailyMinutes
                            let rem = cur % 60
                            if rem < 59 { setPlannedDaily(hours: nil, minutes: rem + 1) }
                        } label: {
                            Image(systemName: "plus.circle.fill").font(.title3)
                                .foregroundStyle((store.settings.plannedDailyMinutes ?? defaultPlannedDailyMinutes) % 60 < 59 ? .blue : .gray.opacity(0.3))
                        }
                        .buttonStyle(.plain)
                    ...
                    Text(localizedText("默认 8 小时 0 分钟。", "Default is 8h 0m.", language: language))
                        ...
                        .foregroundStyle(.secondary)
                    HStack {
                        Text(localizedText("超时额外间隔", "Overtime Extra Interval", language: language))
                        Spacer()
                        Button {
                            let cur = store.settings.overtimeRepeatExtraMinutes ?? 5
                            if cur > 1 {
                                store.settings.overtimeRepeatExtraMinutes = cur - 1
                                ScreenTimeGuardianScreenTimeStorage.sharedDefaults()?.set(cur - 1, forKey: ScreenTimeGuardianScreenTimeStorage.overtimeRepeatExtraMinutesKey)
                            }
                        } label: {
                            Image(systemName: "minus.circle.fill").font(.title3)
                                .foregroundStyle((store.settings.overtimeRepeatExtraMinutes ?? 5) > 1 ? .blue : .gray.opacity(0.3))
                        }
                        .buttonStyle(.plain)
                        ...
                        Button {
                            ...
                            Image(systemName: "plus.circle.fill").font(.title3)
                                .foregroundStyle((store.settings.overtimeRepeatExtraMinutes ?? 5) < 120 ? .blue : .gray.opacity(0.3))
                        }
                        .buttonStyle(.plain)
                        Text(localizedText("分钟", "min", language: language))
                            .foregroundStyle(.secondary)
                    }
                    Text(localizedText("总超时提醒间隔 = 记录间隔 \(store.settings.iosScreenTimeCheckpointIntervalMinutes ?? 2) + 额外 \(store.settings.overtimeRepeatExtraMinutes ?? 5) = \(overtimeTotalIntervalMinutes) 分钟", "Total overtime interval = record interval \(store.settings.iosScreenTimeCheckpointIntervalMinutes ?? 2) + extra \(store.settings.overtimeRepeatExtraMinutes ?? 5) = \(overtimeTotalIntervalMinutes) min", language: language))
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                }
            }
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(localizedText("取消", "Cancel", language: language)) {
                        restoreSnapshot()
                        dismiss()
                    }
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button(localizedText("保存", "Save", language: language)) {
                        saveAndDismiss()
                    }
                }
            }
    ...
    private func setPlannedDaily(hours newHours: Int?, minutes newMinutes: Int?) {
        ...
        // Sync to App Group immediately so Extension sees the update
        let plannedMinutes = store.settings.plannedDailyMinutes ?? defaultPlannedDailyMinutes
        ScreenTimeGuardianScreenTimeStorage.sharedDefaults()?.set(plannedMinutes, forKey: "screen_time_guardian.planned_daily_minutes")
    }
    ...
    private var overtimeRepeatExtraMinutesBinding: Binding<Int> {
        Binding(
            ...
            set: {
                store.settings.overtimeRepeatExtraMinutes = min(120, max(1, $0))
                // Sync to App Group immediately
                ScreenTimeGuardianScreenTimeStorage.sharedDefaults()?.set(store.settings.overtimeRepeatExtraMinutes ?? 5, forKey: ScreenTimeGuardianScreenTimeStorage.overtimeRepeatExtraMinutesKey)
            }
        )
    }

    private var overtimeTotalIntervalMinutes: Int {
        let checkpoint = store.settings.iosScreenTimeCheckpointIntervalMinutes ?? 2
        let extra = store.settings.overtimeRepeatExtraMinutes ?? 5
        ...
        HStack {
            ...
            Button {
                ...
                Image(systemName: "minus.circle.fill")
                    .font(.title3)
                    .foregroundStyle(value.wrappedValue > 1 ? .blue : .gray.opacity(0.3))
            }
            .disabled(value.wrappedValue <= 1)
            .buttonStyle(.plain)
            TextField(localizedText("分钟", "Min", language: language), value: value, format: .number)
                .keyboardType(.numberPad)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 48)
            Button {
                if value.wrappedValue < 1440 { value.wrappedValue += 1 }
            } label: {
                Image(systemName: "plus.circle.fill")
                    .font(.title3)
                    .foregroundStyle(value.wrappedValue < 1440 ? .blue : .gray.opacity(0.3))
            }
            .disabled(value.wrappedValue >= 1440)
            .buttonStyle(.plain)
            Text(localizedText("分钟", "min", language: language))
                .foregroundStyle(.secondary)
        }
