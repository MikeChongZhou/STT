...
final class ScreenTimeGuardianMonitorExtension: DeviceActivityMonitor {
    ...
    private func recordEvent(for event: DeviceActivityEvent.Name, reachedAt: Date, suppressReminder: Bool) {
        ...
        // Skip zero-duration segments (duplicate threshold events at same threshold)
        // But still update appTotalRecordedSeconds if needed for gap compensation.
        if actualSegmentSeconds <= 0 {
            // Even for duplicate checkpoints, check if gap compensation is needed.
            ...
            let gap = currentSystemTotalSeconds - appTotalSeconds
            if gap > 0 {
                // Gap exists — update appTotal but don't create a new record
                defaults2?.set(currentSystemTotalSeconds, forKey: appTotalKey)
            }
            return
        }
    ...
    private func shouldRecordThreshold(thresholdMinutes: Int, now: Date) -> Bool {
        ...
        // Historical catch-up check: ONLY applies to reminder events.
        // Checkpoint events must NEVER be skipped — they record time data and run gap compensation.
        // Skipping a checkpoint would lose screen time segments permanently.
        let isCheckpoint = !isReminderThreshold(thresholdMinutes)
        if !isCheckpoint {
